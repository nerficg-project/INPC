...
namespace inpc::rasterization::training {
        ...
        const float far)
    {
        ...
        features_view[point_idx] = convert_features(features + point_idx * sh_coefficients_per_channel, cam_position[0], position_world);
        ...
        const uint width)
    {
        ...
        for (uint current_base_idx = start_idx; current_base_idx < end_idx && transmittance >= config::transmittance_threshold; current_base_idx += warp_size) {
            ...
            // atomicAdd(&blending_weights[point_idx], blending_weight);
            atomicAdd(&blending_weights[point_idx], blending_weight * blerp_weight);

            // TODO: could use butterfly reduction here
            float4 weighted_features_lane = blending_weight * point_features_view[point_idx];
        ...
        if (leader) {
            alpha[pixel_idx] = transmittance < config::transmittance_threshold ? 1.0f : 1.0f - transmittance;
            image[pixel_idx] = features_pixel.x;
            image[pixel_idx + n_pixels] = features_pixel.y;
            image[pixel_idx + 2 * n_pixels] = features_pixel.z;
            image[pixel_idx + 3 * n_pixels] = features_pixel.w;
        }
    }
