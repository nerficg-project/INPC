...
std::pair<int, int> inpc::rasterization::bilinear::forward(
    ...
    const float far_plane)
{
    ...
    CHECK_CUDA(config::debug_forward, "cub::DeviceRadixSort::SortPairs (Depth)")
    ...
    CHECK_CUDA(config::debug_forward, "apply_depth_ordering")
    ...
    CHECK_CUDA(config::debug_forward, "cub::DeviceScan::ExclusiveSum")
    ...
    CHECK_CUDA(config::debug_forward, "cub::DeviceRadixSort::SortPairs (Tile)")
    ...
    kernels::blend_forward_cu<<<grid, block>>>(
        per_tile_buffers.instance_ranges,
        per_instance_buffers.primitive_indices.Current(),
        opacities,
        per_primitive_buffers.screen_coords,
