...
namespace inpc::rasterization::bilinear::kernels {
        ...
        const float far_plane)
    {
        ...
        constexpr float extent = 0.5f;  // actual extent is 1, but the bounding box only needs to touch the pixels for which evaluation should occur

        const uint4 screen_bounds = make_uint4(
            min(grid_width, static_cast<uint>(max(0, __float2int_rd((screen_coords.x - extent) / static_cast<float>(config::tile_width))))), // x_min
            min(grid_width, static_cast<uint>(max(0, __float2int_ru((screen_coords.x + extent) / static_cast<float>(config::tile_width))))), // x_max
            min(grid_height, static_cast<uint>(max(0, __float2int_rd((screen_coords.y - extent) / static_cast<float>(config::tile_height))))), // y_min
            min(grid_height, static_cast<uint>(max(0, __float2int_ru((screen_coords.y + extent) / static_cast<float>(config::tile_height))))) // y_max
        );
        ...
        const uint grid_width)
    {
        ...
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            ...
            for (int j = 0; !done && j < current_batch_size; ++j) {
                const float2 distance = fabsf(collected_screen_coords[j] - pixel);
                if (distance.x >= 1.0f || distance.y >= 1.0f) continue;
                const float2 blerp_factors = 1.0f - distance;
                const float blerp_weight = __saturatef(blerp_factors.x) * __saturatef(blerp_factors.y);
                const float alpha = collected_opacity[j] * blerp_weight;

                const float blending_weight = transmittance * alpha;
                if (blending_weight > 0.0f) {
                    // summing the four values for each point (intuitive approach) performs slightly worse, multiplying by the blerp weight causes fragments closer to the point's center to have higher influence
                    // atomicAdd(&blending_weights[collected_primitive_indices[j]], blending_weight);
                    atomicAdd(&blending_weights[collected_primitive_indices[j]], blending_weight * blerp_weight);
                }
        ...
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            ...
            for (int j = 0; !done && j < current_batch_size; ++j) {
                ...
                if (blending_weight > 0.0f) {
                    ...
                    constexpr bool incorrect_feat_grads = false;
                    if constexpr (incorrect_feat_grads) weight *= blerp_weight;

                    const float4 dL_dfeatures = weight * grad_features_pixel;
                ...
                const float dL_dopacity = dL_dalpha * blerp_weight;
