...
void inpc::rasterization::gaussian::inference_preextracted(
    ...
    const float sigma_cutoff)
{
    ...
    kernels::preprocess_preextracted_cu<<<div_round_up(n_primitives, config::block_size_preprocess), config::block_size_preprocess>>>(
        positions,
        features,
        opacities,
        scales,
