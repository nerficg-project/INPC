...
void inpc::rasterization::gaussian::inference(
    ...
    const float sigma_cutoff)
{
    ...
    char* per_primitive_buffers_blob = per_primitive_buffers_func(required<PerPrimitiveBuffers>(n_primitives, true));
    PerPrimitiveBuffers per_primitive_buffers = PerPrimitiveBuffers::from_blob(per_primitive_buffers_blob, n_primitives, true);
    ...
    kernels::preprocess_inference_cu<<<div_round_up(n_primitives, config::block_size_preprocess), config::block_size_preprocess>>>(
        positions,
        features_raw,
        w2c,
        cam_position,
    ...
    CHECK_CUDA(config::debug_inference, "preprocess_inference")
    ...
    CHECK_CUDA(config::debug_inference, "cub::DeviceRadixSort::SortPairs (Depth)")
    ...
    CHECK_CUDA(config::debug_inference, "apply_depth_ordering")
    ...
    CHECK_CUDA(config::debug_inference, "cub::DeviceScan::ExclusiveSum")
    ...
    CHECK_CUDA(config::debug_inference, "cub::DeviceRadixSort::SortPairs (Tile)")
    ...
    kernels::blend_inference_cu<<<grid, block>>>(
        ...
        height,
        grid.x,
        expf(-0.5f * sigma_cutoff * sigma_cutoff)
    );
    CHECK_CUDA(config::debug_inference, "blend_inference")
